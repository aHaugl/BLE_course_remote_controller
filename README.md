...
------------ |
<img src="https://github.com/aHaugl/BLE_course_remote_controller/blob/main/images/Step1.5.png" width="1000"> |
...
Congratulations! You have built and flashed our first application. Let's move on by doing some minor modifications. If you explore some of the samples from the *nrf* folder in NCS, you'll see that most of them use our logging module, which is what we will use as well. In order to do so, replace the line `#include <sys/printk.h>` with `#include <zephyr/logging/log.h>`. If the line for printk.h is not present, then just add the logging/log header.

In order to use the log module, we need to add a few things in the prj.conf file. You will find it from the application tab (called remote_controller if you didn't change it) -> Config files -> prj.conf. At this point, it should just say `#nothing here`.
</br>
...
<img src="https://github.com/aHaugl/BLE_course_remote_controller/blob/main/images/Step1.6.png" width="1000"> |
